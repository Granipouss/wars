...
export const coroutine =
    <TArgs extends any[], T = unknown, TReturn = any, TNext = unknown>(
        gen: (...args: TArgs) => Generator<T, TReturn, TNext>,
    ) =>
    (...args: TArgs) => {
        const instance = gen(...args);
        instance.next();
        return (x: TNext) => instance.next(x);
    };

export type Coroutine<T = unknown, TReturn = any, TNext = unknown> = (x: TNext) => IteratorResult<T, TReturn>;

export const wait = (ms: number) => new Promise<void>((resolve) => setTimeout(resolve, ms));

export const stream = <T>(observable: Observable<T>): AsyncGenerator<T, void, void> => {
